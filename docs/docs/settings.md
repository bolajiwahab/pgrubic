...
If default and the environment variable `PGRUBIC_CACHE` is set, the environment variable
takes precedence or otherwise the non-default set value is always used.
...
The target version of Postgres to lint against. This is used to either enable or disable
certain linting rules. For example, `DETACH PARTITION CONCURRENTLY`
...
### **comma-at-beginning**
If `true`, add comma as a prefix as opposed to a suffix when formatting a list of items,
such as list of columns in which each column is on a separate line.
...
are located in the `pg_catalog` schema and thus prefixed with `pg_catalog.` by default.
...
formatted, without making any changes to the file. If there is a difference, it exits with
...
### **check**
When `true`, it exits with a non-zero exit code if the any files would have been modified
by the formatter.
...
### **no-cache**
Whether to read the cache. Caching helps speed up the formatting process. When a file has
not been modified after the last formatting, it is simply skipped.
